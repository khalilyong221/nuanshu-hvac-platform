...
- **账号登录与家庭绑定**：业主首次打开小程序需登录（手机号验证码或微信一键登录，演示环境不发真实短信，演示账号 138 0000 2703 名下有 3 个家庭）。家庭与手机号的绑定关系由服务商在中控台维护：新建家庭、录入设备、绑定或解绑业主手机号。业主登录后在首页顶部的家庭栏查看「我的家庭」并切换，可以修改家庭名称和地址（中控台与硬件面板同步显示），但不能自行增删家庭——需提交「申请添加 / 移除家庭」，由中控台在待处理项目中处理，结果回传业主端。同一浏览器内通过 localStorage 实时同步。
