...
- **智能家居与安防（三端同步）**：星海国际公寓接入智能门锁（开关门动作检测）、分区摄像头、烟雾报警器、燃气探测器、智能窗帘（滑动长条无级调节开合，滑到最左自动关闭、向右滑动自动打开，开关一键恢复上次位置）和分区灯带 / 筒灯。业主端与硬件面板可以操作，服务商中控台实时查看并可对报警一键派单；在同一浏览器中同时打开三端页面，状态会通过 localStorage 实时同步。
